    ...
    <div>
      ...
      <div className="path-points">
        {pathData.path.map((point, index) => (
          <div key={index} data-testid="path-point">
            {`(${point.x}, ${point.y})`}
          </div>
        ))}
      </div>
      ...
      <div className="statistics" data-testid="statistics">
        <div data-testid="total-distance">
          {`${formatNumber(pathData.statistics.totalDistance)} meters`}
        </div>
        <div data-testid="max-slope">
          {`${formatNumber(pathData.statistics.maxSlope)}°`}
        </div>
        <div data-testid="elevation-gain">
          {`${formatNumber(pathData.statistics.elevationGain)} meters`}
        </div>
