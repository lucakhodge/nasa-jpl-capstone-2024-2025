...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::pair<std::pair<int, int>, std::pair<int, int>> rectCoordinates(
        imageCoordinates, imageCoordinates2);
  ...
  try {
    ...
    mempa::RoverSimulator marsSimulator(&marsRaster, userCoordinates1,
                                        userCoordinates2);
    ...
    if (!ci_env) // If CI variable is not set
    {
      for (std::pair<int, int> pathCoordinate : routedPath) {
        std::cout << "(" << pathCoordinate.first << ", "
                  << pathCoordinate.second << ")\n";
      }
      std::cout << '\n';
      for (std::pair<int, int> pathCoordinate : routedPath) {
        std::pair<double, double> geoCoordinate =
            marsRaster.revertCoordinates(pathCoordinate);
        std::cout << std::fixed << std::setprecision(4) << "("
                  << geoCoordinate.first << ", " << geoCoordinate.second
                  << ")\n";
      }
    }
  } catch (const std::exception &demError) {
