...
public:
   ...
   */
  void
  calculateHorizontalDistance(const std::vector<std::pair<int, int>> &path);
  ...
  void
  calculateTotalElevationChange(const std::vector<std::pair<int, int>> &path,
                                const mempa::DemHandler *demHandler);
  ...
  void calculateAverageSlope(const std::vector<std::pair<int, int>> &path,
                             const mempa::DemHandler *demHandler);
  void calculateElevationGain(const std::vector<std::pair<int, int>> &path,
                              const mempa::DemHandler *demHandler);
  void calculateElevationLoss(const std::vector<std::pair<int, int>> &path,
                              const mempa::DemHandler *demHandler);

  /**
   * @brief Legacy path analysis method (note: maintains misspelling for
