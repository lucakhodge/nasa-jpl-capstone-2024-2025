...
constexpr double MAX_ELEVATION_CHANGE = 5.0; // Maximum elevation change per pixel (meters)
constexpr double ELEVATION_EPSILON = 0.01;   // Small elevation difference threshold (meters)
...
struct ElevationTracker
{
  ...
  ElevationTracker() : baseElevation(std::numeric_limits<double>::quiet_NaN()), warningCount(0), rows(0), cols(0) {}
...
std::pair<int, int> Dijkstras::get_step(
