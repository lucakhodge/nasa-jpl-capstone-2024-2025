...
namespace mempa
{
    ...
    std::vector<std::pair<int, int>> RoverSimulator::runSimulator(const int buffer)
    {
        std::vector<std::pair<int, int>> routedPath; /* Holds the coordinates of every traversed area of the raster. */
        routedPath.reserve(std::max(std::abs(endPosition.first - startPosition.first), std::abs(endPosition.second - startPosition.second)));
        ...
        do
        {
            ...
            std::vector<std::vector<float>> elevationMap = elevationRaster->readRectangleChunk(regionCoordinates, buffer, &vectorCoordinates); /* The elevation data we read from the DemHandler elevationRaster. */
            std::vector<std::vector<float>> slopeMap = slopeRaster->readRectangleChunk(regionCoordinates, buffer);                             /* Corresponding slope values for each point of elevation data. */

            if (!(validateCoordinate(vectorCoordinates.first, elevationMap) && validateCoordinate(vectorCoordinates.second, elevationMap)))
            {
                throw std::runtime_error("runSimulator: startPosition or endPosition out of bounds");
            }
