...
// #include "mempa/algorithm/src/mempa_dem_handler/mempa_dem/buf_dem.h" // TODO: relative path
...
const double MAX_ELEVATION_CHANGE = 50.0;  // Maximum elevation change per pixel (meters)
const double ELEVATION_EPSILON = 0.1;      // Small elevation difference threshold (meters)
...
const double SIGNIFICANT_CHANGE = 10.0;    // Significant elevation change threshold (meters)
...
public:
    void updateProgress(const std::string& status, double progress) {
        std::cout << "\r" << spinner[spinnerIndex] << " " 
...
private:
    ...
    bool isValidCoordinate(const std::pair<int, int>& coord, 
        ...
        bool valid = coord.first >= 0 && coord.first < static_cast<int>(heightmap.size()) &&
        coord.second >= 0 && coord.second < static_cast<int>(heightmap[0].size());
    ...
    double validateElevation(double rawElevation) {
        ...
        if (elevation > 1000000.0) {
            ...
            logDebug("Scaled large elevation from " + 
                    std::to_string(elevation/ELEVATION_SCALING) + 
                    "m to " + std::to_string(elevation) + "m");
        }
    ...
    double processElevation(double rawElevation, int row, int col) {
        ...
        if (!metrics.baseElevationSet) {
            ...
            logDebug("Base elevation set to: " + std::to_string(metrics.baseElevation) + "m");
            return 0.0;
        }
    ...
    double calculateSlope(double dx, double dy, double elevChange) {
        ...
        if (horizontalDist < ELEVATION_EPSILON) return 0.0;
    ...
    std::pair<int, int> convertToLocalCoordinates(int globalX, int globalY, 
        ...
        logDebug("Converting global (" + std::to_string(globalX) + "," + 
                std::to_string(globalY) + ") to local (" + 
    ...
    void optimizePath(std::vector<std::pair<int, int>>& path) {
        ...
        double lastSignificantElev = metrics.relativeHeights[path[0].first][path[0].second];
        ...
        logDebug("Path optimized from " + std::to_string(originalSize) + 
                 " to " + std::to_string(path.size()) + " waypoints");
    }
    ...
    void updatePathMetrics(const std::vector<std::pair<int, int>>& path, 
        ...
        try {
            ...
            double startElev = processElevation(heightmap[path[0].first][path[0].second], 
            path[0].first, path[0].second);
            double endElev = processElevation(heightmap[path.back().first][path.back().second], 
            ...
            for (size_t i = 1; i < path.size(); ++i) {
                ...
                metrics.energyCost += metrics.calculateEnergyCost(distance, elevChange, slope);
            ...
            if (!metrics.slopes.empty()) {
                metrics.averageSlope = std::accumulate(metrics.slopes.begin(), 
                metrics.slopes.end(), 0.0) / 
                metrics.slopes.size();
            }
    ...
    void displayDetailedSummary(int startX, int startY, int endX, int endY) {
        ...
        const double actualDistance = std::sqrt(
            std::pow((endX - startX) * metersPerPixel, 2) +
    ...
    void displayResults() {
        std::cout << "\nSimulation Results:\n"
                  ...
                  << "Total Elevation Change: " << metrics.totalElevationChange << " m\n"
                  << "Net Elevation Change: " << metrics.netElevationChange << " m\n"
...
public:
    ...
    void run(int startX, int startY, int endX, int endY) {
        ...
        const std::string demFilePath = "/Users/adamcarlson/Dropbox/Mac/Desktop/Desktop/"
                                        "nasa-jpl-capstone-2024-2025/prototypes/sim/"
                                        "Mars_HRSC_MOLA_BlendDEM_Global_200mp_v2.tif";
        const std::string outputPath = "/Users/adamcarlson/Dropbox/Mac/Downloads";
        ...
        try {
            ...
            uint32_t startRow = (startY > TERRAIN_PADDING) ? startY - TERRAIN_PADDING : 0;
            uint32_t endRow = std::min(static_cast<uint32_t>(endY + TERRAIN_PADDING), 
                                     53347u);
            uint32_t startCol = (startX > TERRAIN_PADDING) ? startX - TERRAIN_PADDING : 0;
            uint32_t endCol = std::min(static_cast<uint32_t>(endX + TERRAIN_PADDING), 
                                     106694u);

            logDebug("Loading heightmap for region: (" + std::to_string(startRow) + "," + 
                    std::to_string(startCol) + ") to (" + std::to_string(endRow) + "," + 
                    std::to_string(endCol) + ")");

            auto heightmap = demHandler.demVector(demFilePath, startRow, endRow, 
                                               startCol, endCol);
            ...
            auto localStart = convertToLocalCoordinates(startX, startY, startRow, startCol);
            ...
            optimizePath(path);
            writePathData("rover_path.txt", path, heightmap, startX, startY, endX, endY,
            startRow, startCol);
...
int main(int argc, char** argv) {
    try {
        ...
        if (argc >= 5) {
            ...
            std::cerr << "Usage: " << argv[0] << " <startX> <startY> <endX> <endY>\n"
                     << "Valid ranges:\n"
                     << "  X: 0 to 106,694\n"
                     << "  Y: 0 to 53,347\n"
        ...
        if (startX < 0 || startY < 0 || endX < 0 || endY < 0 ||
            startX > 106694 || endX > 106694 ||
            startY > 53347 || endY > 53347) {
        ...
        sim.run(startX, startY, endX, endY);
        return 0;
